...
function getEncryptionKey(): Buffer {
  const secret = process.env.SMS_ENCRYPTION_SECRET || process.env.ENCRYPTION_KEY || process.env.SESSION_SECRET || 'eduerp-production-sms-vault-secret-key-2026';
...
export function decryptSmsCredentials(encryptedString: string): Record<string, any> {
  ...
  if (parts.length !== 4 || parts[0] !== 'v1') {
    throw new Error('Invalid or corrupted SMS credential format.');
  }
