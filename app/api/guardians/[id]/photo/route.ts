...
export async function POST(
  ...
  try {
    ...
    const relationRole = ((formData.get('relationRole') as string) || 'FATHER') as 'FATHER' | 'MOTHER' | 'GUARDIAN';
    ...
    const result = await MediaStorageService.updateGuardianPhoto(
      tenantSlug,
      guardianId,
      relationRole,
      buffer,
    ...
    return NextResponse.json(
      { success: false, error: error.message || 'Failed to update guardian photo.' },
...
export async function DELETE(
  ...
  try {
    ...
    const relationRole = (url.searchParams.get('relationRole') || 'FATHER') as 'FATHER' | 'MOTHER' | 'GUARDIAN';
    ...
    await MediaStorageService.removeGuardianPhoto(tenantSlug, guardianId, relationRole, session.userId);
    ...
    return NextResponse.json(
      { success: false, error: error.message || 'Failed to remove guardian photo.' },
      { status: error.statusCode || 400 }
    );
  }
}
