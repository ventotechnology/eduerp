...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!tenantId) {
      throw AppError.notFound('Tenant context is required.');
    }

    if (!session.isPlatformAdmin && session.tenantId !== tenantId) {
      throw AppError.crossTenant();
    }

    const result = await getTenantStudents(tenantId, {
...
export async function POST(req: NextRequest) {
  try {
    ...
    const student = await createTenantStudent(tenantId, body, session);
    return successResponse(student, 'Student record created successfully', 201);
  } catch (err) {
    return errorResponse(err);
  }
}
